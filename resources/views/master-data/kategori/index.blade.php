...
@section('content')
    <x-ui.page-header title="Kategori Barang" description="Atur kelompok barang agar inventaris lebih mudah dibaca.">
    ...
    <form id="bulk-form" method="POST" action="{{ route('master-data.kategori.bulk-destroy') }}"
          data-confirm="Hapus semua kategori yang dipilih? Tindakan ini tidak dapat dibatalkan.">
        @csrf
        @method('DELETE')
    </form>
    ...
    <section class="surface overflow-hidden">
        <form method="GET" class="border-b border-slate-200 px-5 py-4">
            <div class="grid gap-3 md:grid-cols-[minmax(0,1fr)_auto_auto]">
                <div class="relative">
                    ...
                    <input type="text" name="q" value="{{ $q }}" class="input-field pl-11" placeholder="Cari nama atau deskripsi kategori">
                </div>
                ...
                <div class="flex gap-3">
                    ...
                    @if ($q)
                        <a href="{{ route('master-data.kategori.index') }}" class="btn btn-secondary">Reset</a>
                    @endif
                </div>
            </div>
        </form>
        ...
        @if ($kategori->isEmpty())
            <x-ui.empty-state title="Kategori belum tersedia" description="Tambahkan kategori pertama untuk mulai mengelompokkan barang." icon="package" />
        @else
                ...
                <table class="data-table">
                    <thead>
                        <tr>
                            ...
                            <x-ui.sortable-th column="nama" label="Nama Kategori" :sort-by="$sortBy" :sort-dir="$sortDir" />
                            <th>Deskripsi</th>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="flex justify-center gap-2">
                                        <a href="{{ route('master-data.kategori.edit', $item) }}" class="btn btn-secondary px-3 py-2">
                                            <x-ui.icon name="pencil" class="h-4 w-4" />
                                        </a>
                                        <form method="POST" action="{{ route('master-data.kategori.destroy', $item) }}" data-confirm="Hapus kategori '{{ $item->nama }}'?">
                                            @csrf
                                            @method('DELETE')
