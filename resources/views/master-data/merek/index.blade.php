...
@section('content')
    <x-ui.page-header title="Merek Barang" description="Kelola daftar merek agar analisis barang lebih jelas.">
        ...
        <a href="{{ route('master-data.merek.create') }}" class="btn btn-primary">
            <x-ui.icon name="plus" class="h-4 w-4" />
            <span>Tambah Merek</span>
        </a>
    </x-ui.page-header>

    <form id="bulk-form" method="POST" action="{{ route('master-data.merek.bulk-destroy') }}"
          data-confirm="Hapus semua merek yang dipilih? Tindakan ini tidak dapat dibatalkan.">
        @csrf
        @method('DELETE')
    </form>
    ...
    <section class="surface overflow-hidden">
        <form method="GET" class="border-b border-slate-200 px-5 py-4">
            <div class="grid gap-3 md:grid-cols-[minmax(0,1fr)_auto_auto]">
                <div class="relative">
                    ...
                    <input type="text" name="q" value="{{ $q }}" class="input-field pl-11" placeholder="Cari merek barang">
                </div>
        ...
        @if ($merek->isEmpty())
            <x-ui.empty-state title="Merek belum tersedia" description="Tambahkan merek untuk melengkapi data barang." icon="package" />
        @else
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="flex justify-center gap-2">
                                        <a href="{{ route('master-data.merek.edit', $item) }}" class="btn btn-secondary px-3 py-2">
                                            <x-ui.icon name="pencil" class="h-4 w-4" />
                                        </a>
                                        <form method="POST" action="{{ route('master-data.merek.destroy', $item) }}" data-confirm="Hapus merek '{{ $item->nama }}'?">
                                            @csrf
                                            @method('DELETE')
